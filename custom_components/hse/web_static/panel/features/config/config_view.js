/**
 * config_view.js — Onglet Configuration HSE V3  (UI V2 — inspiré V1)
 *
 * 3 sous-onglets :
 *   A — Appareils       : groupement par intégration, collapse/expand, badges qualité, triage bulk
 *   B — Pièces & Types  : GET /api/hse/meta + sync/preview + sync/apply
 *   C — Tarification    : layout 2-colonnes HT|TTC, abonnement HT+TTC, plage HC, prévisualisation live
 *
...
const CATALOGUE_PER_PAGE = 200; // on récupère plus pour grouper côté client
...
export class ConfigView {
  constructor() {
    ...
    this._openGroups   = new Set(); // groupes ouverts
  ...
  // ── PANEL A — Appareils (groupes par intégration) ───────────────────────────
  ...
  _renderCatalogue() {
    ...
    // Ouvrir tous les groupes par défaut si vide
  ...
  _renderSensorRow(item) {
    ...
    const statusLbl  = { selected:'S\u00e9lectionn\u00e9', ignored:'Ignor\u00e9', pending:'En attente', normal:'Normal' }[item.status] ?? (item.status ?? 'En attente');
  ...
  _bindGroupEvents() {
    ...
    body.querySelectorAll('[data-grp-sel]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        items.forEach(i => {
          const row = body.querySelector(`[data-eid="${CSS.escape ? CSS.escape(i.entity_id) : i.entity_id}"]`);
          const cb  = body.querySelector(`[data-cb-eid="${CSS.escape ? CSS.escape(i.entity_id) : i.entity_id}"]`);
  ...
  _renderMeta() {
    ...
    const renderList = (items, labelKey) => {
      if (!items.length) return `<li style="padding:12px 14px;color:rgba(255,255,255,0.3);font-size:0.875rem">Aucun \u00e9l\u00e9ment.</li>`;
      return items.map(i => `<li><span>${this._esc(i[labelKey] ?? i.id ?? i.name ?? '?')}</span></li>`).join('');
    };
    this._setMetaBody(`
  ...
  // ── PANEL C — Tarification (V2 : 2-colonnes HT|TTC, plage HC) ───────────────────
  async _loadPricing() {
  ...
  _updatePricingPreview() {
    ...
    const subTtc   = subHt * mult;

    // Mettre à jour les affichages TTC en temps réel
    const setTtc = (id, ht) => {
      const el = form.querySelector(id);
      if (el) el.textContent = ht > 0 ? (ht * mult).toFixed(4) : '\u2014';
    };
    const setTtcVal = (id, val) => {
      const el = form.querySelector(id);
      if (el) el.textContent = val > 0 ? (val * mult).toFixed(3) : '\u2014';
    };
    setTtcVal('#hse-cfg-sub-ttc', subHt);
    ...
    // Preview co\u00fbt mensuel
    const val = this._el?.querySelector('#hse-cfg-preview-val');
    if (!val) return;
    let price = 0;
    if (contract === 'hphc') {
      const priceHp = parseFloat(form.querySelector('#hse-cfg-price-hp')?.value) || 0;
      price = priceHp * mult; // approx HP seul si pas HC estimé séparément
    } else {
      price = (parseFloat(form.querySelector('#hse-cfg-price-ht')?.value) || 0) * mult;
    }
    const total = (kwh * price) + subTtc;
  ...
  async _savePricing(form) {
    ...
    // Compatibilité : subscription_monthly = subscription_ht si présent
